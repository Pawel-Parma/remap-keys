...
5. Run the script with the command `python main.py`. The script will print the AutoHotkey code for the new key mappings.
6. Copy the output and paste it into the `remap.ahk` file below the `; Mapping` section without overriding `mapping := Object()`.
